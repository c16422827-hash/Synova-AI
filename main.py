...
async def chat(req: ChatRequest) -> Dict[str, str]:
    ...
    logger.info(f"Received chat request - Tier: {tier}, Query: {q[:50]}...")
    ...
    try:
        ...
        if is_postgres:
            c.execute('INSERT INTO messages (tier, query, response, timestamp) VALUES (%s,%s,%s,%s)',
                      (tier, q, response, datetime.datetime.utcnow().isoformat()))
        else:
            c.execute('INSERT INTO messages (tier, query, response, timestamp) VALUES (?,?,?,?)',
                      (tier, q, response, datetime.datetime.utcnow().isoformat()))
